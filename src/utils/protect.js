...
export async function requireAuth(request, requiredRole = null) {
    ...
    if (!activeUser) {
        const response = redirect(`/users/login?message=You must login first&redirectTo=${pathname}`);
        return response;
    }
    ...
    if (requiredRole && activeUser.role !== requiredRole) {
        ...
        return redirect(`/users/login?message=Unauthorized access&redirectTo=${pathname}`);
